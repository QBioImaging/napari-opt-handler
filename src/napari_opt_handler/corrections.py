...
class Correct:
    ...
    def correct_dark_bright(
        ...
        if modality == "Transmission":
            if useBright and useDark:
                data_corr = (stack - self.dark) / (self.bright - self.dark)
    ...
    def get_bad_pxs(self, mode: str = "hot") -> List[Tuple[int, int]]:
        ...
        if mode == "hot" or mode == "both":
            ...
            if np.all(self.maskAbove.mask is False):
                ...
                self.logger.info(
                    "%s, %s", str(self.bad.max(), str(self.bad.min()))
                )
            else:
    ...
    def clip_and_convert_data(self, data_corr: np.ndarray) -> np.ndarray:
        ...
        if np.amax(data_corr) > 1 or np.amin(data_corr) < 0:
            self.logger.warning(
                "Overflows %.2f, %.2f, clipping to 0-1.",
                data_corr.min(),
                data_corr.max(),
            )
    ...
    def subtract_images(
        self, image: np.ndarray, corr: np.ndarray
    ) -> np.ndarray:
        ...
        """
        if not np.issubdtype(image.dtype, np.integer) or not np.issubdtype(
            corr.dtype, np.integer
        ):
