...
@pytest.mark.parametrize(
    ...
    [("backtrack_true_false", np.ones((5, 5)), (False, {})),
     ...
     ("backtrack_true_true", np.ones((5, 5)), (True, {'operation': 'roi',
                                                      'data': np.ones((5, 5)),
                                                      'roi_def': ()})),
...
class TestClass:
    def test_update_history_roi(self, make_napari_viewer, backtrack_init,
...
@pytest.mark.parametrize(
    ...
    [("backtrack_true_false", (np.ones((5, 5)), 4), (False, {})),
     ...
     ("backtrack_true_true", (np.ones((5, 5)), 4), (True, {'operation': 'bin',
                                                           'data': np.ones((5, 5)),
                                                           'bin_factor': 2})),
     ("backtrack_true_true", (np.ones((6, 6)), 9), (True, {'operation': 'bin',
                                                           'data': np.ones((6, 6)),
                                                           'bin_factor': 3})),
     ],
)
def test_update_history_bin1(make_napari_viewer, backtrack_init,
                             input_vals, expected, request):
    backtrack = request.getfixturevalue(backtrack_init)
    assert backtrack.raw_data is None
    assert backtrack.roi_def == ()
    ...
    assert np.array_equal(out, data_dict['data'])
    assert type(out) is type(data_dict['data'])
    assert np.array_equal(backtrack.raw_data, viewer.layers['test'].data)
    np.testing.assert_equal(backtrack.history_item, expected[1])
...
@pytest.mark.parametrize(
    ...
    [("backtrack_true_false", np.ones((5, 5)), (False, {})),
     ("backtrack_none_none", np.ones((5, 5)), (False, {})),
     ("backtrack_true_true", np.ones((5, 5)), (True, {'operation': 'corrInt',
                                                      'data': np.ones((5, 5)),
                                                      'rect_dim': 50})),
     ("backtrack_true_true", np.ones((6, 6)), (True, {'operation': 'corrInt',
                                                      'data': np.ones((6, 6)),
                                                      'rect_dim': 50})),
     ],
)
def test_update_history_corrInt(make_napari_viewer, backtrack_init,
                                input_vals, expected, request):
    backtrack = request.getfixturevalue(backtrack_init)
    assert backtrack.raw_data is None
    assert backtrack.roi_def == ()
...
@pytest.mark.parametrize(
    'backtrack_init, input_vals, expected',
    [("backtrack_true_true", 2, (True, {'operation': 'bin',
                                        'data': np.ones((10, 5, 5))*10,
                                        'bin_factor': 2,
                                        }, np.ones((10, 2, 2)) * 10),),
     ("backtrack_true_true", 3, (True, {'operation': 'bin',
                                        'data': np.ones((10, 5, 5))*10,
                                        'bin_factor': 3,
                                        }, np.ones((10, 1, 1)) * 10),),
     ],
...
@pytest.mark.parametrize(
    ...
    [("backtrack_true_true",
      np.array([[1, 1]]),
      (True, {'operation': 'roi', 'data': np.ones((10, 5, 5))*10, 'roi_def': ()},
       np.ones((10, 4, 4)) * 10, (1, 5, 1, 5))),
     ("backtrack_true_true",
      np.array([[2, 1]]),
      (True, {'operation': 'roi', 'data': np.ones((10, 5, 5))*10, 'roi_def': ()},
       np.ones((10, 3, 4)) * 10, (2, 5, 1, 5))),
     ],
)
