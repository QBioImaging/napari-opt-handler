...
@pytest.mark.parametrize(
    ...
    [((), (None, 7, None, None)),  # no xorrections
     ...
     ((hot2, -4.85, ), (None, 7.0, None, None)),
     ((hot2, 0, ), (None, 7.0, None, None)),
     ],
)
...
@pytest.mark.parametrize(
    ...
    [
     ...
     # no action, because img is flat
     ((hot3, 2, ), 'dead', 'n4', img_bad,
      (img_bad_dead_corr_n4, [], [(1, 2)])),

     ((hot3, ), 'both', 'n4', img1, (img1, [], [])),
     # only hot corrected
     ((hot3, 4, ), 'both', 'n4', img_bad, (img_bad_hot_corr_n4, [(2, 1)], [])),
     # both corrected
     ...
     ((hot3, 2, ), 'dead', 'n8', img_bad,
      (img_bad_dead_corr_n8, [], [(1, 2)])),

     ((hot3, ), 'both', 'n8', img1, (img1, [], [])),
     ((hot3, 4, ), 'both', 'n8', img_bad, (img_bad_hot_corr_n8, [(2, 1)], [])),
...
@pytest.mark.parametrize(
    ...
    [
     ((hot2, 4, dark1, bright2), (stack1,),
      np.ones((5, 5, 5)) * 4),
     # explicit inputs
     ((hot2, 4, dark1, bright2), (stack1, True, False, 'Transmission'),
      np.ones((5, 5, 5)) * 4),
     ((hot2, 4, dark1, bright2), (stack1, True, False, 'Emission'),
      np.ones((5, 5, 5)) * 4),
     # no dark, should return a copy
     ((hot2, 4, dark1, bright2), (stack1, False, False, 'Transmission'),
      stack1),
     ((hot2, 4, dark1, bright2), (stack1, False, False, 'Emission'),
      stack1),

     # now if bright is used
     ((hot2, 4, dark1, bright2), (stack1, True, True, 'Transmission'),
      (np.ones((5, 5, 5)) * 4 / 9 * (2**16 - 1)).astype(np.uint16)),
     ((hot2, 4, dark1, bright2), (stack1, True, True, 'Emission'),
      (np.zeros((5, 5, 5))).astype(np.uint16)),

     # no dark
     ((hot2, 4, dark1, bright2), (stack1, False, True, 'Transmission'),
      (np.ones((5, 5, 5)) * 5 / 10 * (2**16 - 1)).astype(np.uint16)),
     ((hot2, 4, dark1, bright2), (stack1, False, True, 'Emission'),
      (np.zeros((5, 5, 5))).astype(np.uint16)),
     ],
)
def test_correct_dark_bright(inits, corr_inputs, expected):
    corr = Correct(*inits)
    corrected = corr.correct_dark_bright(*corr_inputs,)
    assert np.array_equal(corrected, expected)
...
@pytest.mark.parametrize(
    'inits, corr_inputs, expected',
    [
     ((hot2, 4, dark1, bright2), (stack1, True, False, None),
      'Unknown modality option, valid is Transmission and Emission.'),
     ((hot2, 4, dark1, bright2), (stack1, True, False, 'Nonsence'),
      'Unknown modality option, valid is Transmission and Emission.'),
    ]
)
def test_correct_dark_bright_except(inits, corr_inputs, expected):
    ...
    with pytest.raises(ValueError, match=expected):
        corr.correct_dark_bright(*corr_inputs,)
...
@pytest.mark.parametrize(
    ...
    [
     ...
     ((hot2, 4, dark1, bright2), (stack1,),
      (return_stack1, {'mode': 'integral', 'ref': 10.})),
     # explicit inputs
     ((hot2, 4, dark1, bright2), (stack1, 'integral', True, 50, True),
      (return_stack1, {'mode': 'integral', 'ref': 10.})),
     # no bright correction
     ((hot2, 4, dark1, bright2), (stack1, 'integral', False, 50, True),
      (stack1, {'mode': 'integral', 'ref': 5.})),
     ((hot2, -1, dark1, bright2), (stack1, 'integral', False, 2, True),
      (stack1, {'mode': 'integral', 'ref': 5.})),
     ],
)
...
@pytest.mark.parametrize(
    'input_vals, expected',
    [({'data1': np.ones((10, 5, 5)) * 10, 'data2': np.ones((10, 5, 5)) * 5},
      np.ones((10, 5, 5)) * 5),
     ({'data1': np.ones((10, 5, 5)) * 10, 'data2': np.ones((10, 5, 5)) * 10},
      np.zeros((10, 5, 5))),
     ({'data1': np.ones((10, 5, 5)) * 10, 'data2': np.ones((10, 5, 5)) * 15},
      np.zeros((10, 5, 5))),
     ],
)
...
def test_set_std_mult(input_vals: dict, expected: float):
    ...
    if input_vals['std_mult'] <= 0:
        with pytest.raises(ValueError,
                           match='STD multiplier should be positive'):
